...
def safe_load(f, map_location):
    """Load a pickle file that may contain CUDA tensors, remapping to map_location."""

    class _RemapUnpickler(pickle.Unpickler):
        def find_class(self, module, name):
            if module == "torch.storage" and name == "_load_from_bytes":
                return lambda b: torch.load(io.BytesIO(b), map_location=map_location)
...
class BotRGCN(nn.Module):
    def __init__(self, args):
        ...
        self.linear_relu_tweet = nn.Sequential(
            nn.Linear(self.tweet_size, int(input_dimension / 4)), nn.LeakyReLU()
        )
        self.linear_relu_num_prop = nn.Sequential(
            nn.Linear(self.num_prop_size, int(input_dimension / 4)), nn.LeakyReLU()
        )
        self.linear_relu_cat_prop = nn.Sequential(
            nn.Linear(self.cat_prop_size, int(input_dimension / 4)), nn.LeakyReLU()
        )
    ...
    def forward(self, data, return_attention=False):
        ...
        cat_prop = x[
            :,
            self.num_prop_size
            + self.tweet_size : self.num_prop_size
            + self.tweet_size
            + self.cat_prop_size,
        ]
...
class StructBehavior(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            loss = F.cross_entropy(train_out, self.idx["train_label"])
            loss = (
                loss
            ...
            val_f1 = f1_score(val_label_np, val_pred)

            test_out = fused_feat[self.idx["test_idx"]]
            test_out = self.classifier(test_out)
            test_loss = F.cross_entropy(test_out, self.idx["test_label"])
...
class Trainer(object):
    ...
    def load_data(self):
        ...
        attri_raw_path = os.path.join(PWD, "Data/TwiBot20/attribute_tree/mid")
        tree_path = os.path.join(
            PWD,
            "Data/TwiBot20/attribute_tree/trees",
        ...
        with open(
            ...
            self.knn_edge_index_list = safe_load(fp2, "cpu")

        with open(tree_path, "rb") as fp1:
            self.layer_data = safe_load(fp1, "cpu")
        ...
        attribute_pool_graph_path = os.path.join(
            PWD, "Data/TwiBot20/attribute_pool/twibot20_weighted_graph.dgl"
        )
        ...
        map_data_path = os.path.join(PWD, "Data/TwiBot20/attribute_pool/data_df.csv")
        map_data = pd.read_csv(map_data_path)
        args.map_data = map_data[0:11826]

        whole_raw_path = os.path.join(PWD, "Data/TwiBot20/material_data")
        label = torch.load(os.path.join(whole_raw_path, "label.pt"))
        x = torch.cat(
            [
                torch.load(os.path.join(whole_raw_path, "num_properties_tensor.pt")),
                torch.load(os.path.join(whole_raw_path, "tweets_tensor.pt")),
                torch.load(os.path.join(whole_raw_path, "cat_properties_tensor.pt")),
                torch.load(os.path.join(whole_raw_path, "des_tensor.pt")),
            ],
            dim=1,
        )
        edge_index = torch.load(os.path.join(whole_raw_path, "edge_index.pt"))
        edge_type = torch.load(os.path.join(whole_raw_path, "edge_type.pt"))
        ...
        train_idx = sample_idx[: int(0.7 * args.node_num)]
        val_idx = sample_idx[int(0.7 * args.node_num) : int(0.9 * args.node_num)]
        test_idx = sample_idx[int(0.9 * args.node_num) :]

        train_label = label[: int(0.7 * args.node_num)].to(self.args.device)
        val_label = label[int(0.7 * args.node_num) : int(0.9 * args.node_num)].to(
            self.args.device
        )
        test_label = label[int(0.9 * args.node_num) :].to(self.args.device)
    ...
    def organize_val_log(self, val_loss, val_acc, val_f1, epoch):
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            print(
                "epoch: %d, val_acc: %.4f, val_loss: %.4f, val_f1: %.4f, test_acc: %.4f, test_precision: %.4f, test_recall: %.4f, test_f1: %.4f%s"
                % (
                    epoch,
                    val_acc,
                    val_loss,
            ...
            self.organize_val_log(val_loss, val_acc, val_f1, epoch)
        ...
        val_f1s_np = np.array(val_f1s)
        best_idx = int(np.argmax(val_f1s_np))
        test_results = self.model.get_test_results()
        best_result = test_results[best_idx]
        print("=== Best epoch by val_f1 ===")
        print(
            "best_epoch: %d, best_val_f1: %.4f, test_acc_at_best: %.4f, test_f1_at_best: %.4f, test_recall_at_best: %.4f, test_precision_at_best: %.4f"
            % (
                best_idx,
                val_f1s_np[best_idx],
                best_result[0],
        ...
        top_k = min(self.save_top_k, len(val_f1s_np))
        top_f1_index = val_f1s_np.argsort()[::-1][:top_k]
        print("=== top-%d results by val_f1 ===" % top_k)
        for idx in top_f1_index:
            result = test_results[idx]
            print(
                "epoch: %d, val_f1: %.4f, test_acc: %.4f, test_f1: %.4f, test_recall: %.4f, test_precision: %.4f"
                % (idx, val_f1s_np[idx], result[0], result[1], result[2], result[3])
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--attribute_num", type=int, default=39)
