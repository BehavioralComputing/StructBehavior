...
class AttributePool(nn.Module):
    ...
    def forward(self, g: dgl.DGLGraph):
        ...
        with g.local_scope():
            g.ndata['h'] = h
            all_mean_feat = torch.zeros(args.node_num, self.hidden_dim).to(self.device)
            for index, row in self.map_data.iterrows():
                if index >= args.node_num:
                    break
                feat_sum = torch.zeros(self.hidden_dim).to(self.device)
                num = 0
                for feature in self.map_data.columns:
                    if pd.notna(row[feature]):
                        num = num + 1
                        feat_sum += h[feature][row[feature]]
...
class StructBehavior(nn.Module):
    ...
    def forward(self, batch):
        ...
        loss_1_3 = self.infonce_loss(
            self.proj_1_3(out_1), self.proj_1_3(out_3), self.args.temperature
        )
        ...
        if self.training:
            ...
            loss = loss + loss_1_2 * self.args.alpha_1_2 + loss_1_3 * self.args.alpha_1_3
            return loss
        else:
            # Validation
...
class Trainer(object):
    ...
    def load_data(self):
        ...
        tree_path = os.path.join(
            PWD, 'Data/TwiBot20/attribute_tree/trees',
        ...
        # Layer2 Data
        attribute_pool_graph_path = os.path.join(PWD, 'Data/TwiBot20/attribute_pool/twi_weighted_graph.dgl')
        ...
        val_label = label[int(0.7 * args.node_num) : int(0.9 * args.node_num)].to(self.args.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alpha_1_3', type=float, default = 0.1)
