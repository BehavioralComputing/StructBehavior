...
class StructBehavior(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            val_prob = F.softmax(val_out, dim=1)[:, 1].detach().cpu().numpy()
            val_auc = roc_auc_score(val_label_np, val_prob)
            ...
            test_prob = F.softmax(test_out, dim=1)[:, 1].detach().cpu().numpy()
            auc_roc = roc_auc_score(test_label, test_prob)

            self.test_results.append(
                [test_acc, test_f1, test_recall, test_precision, auc_roc]
            )
            return (
                val_acc,
                val_loss.item(),
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.best_val_auc = -1e9
        self.best_val_auc_epoch = 0
        self.test_results = []

    def load_data(self):

        attri_raw_path = os.path.join(PWD, "Data/BAF/attribute_tree/mid")
        tree_path = os.path.join(
            PWD,
            "Data/BAF/attribute_tree/trees",
        ...
        attribute_pool_graph_path = os.path.join(
            PWD, "Data/BAF/attribute_pool/baf_weighted_graph.dgl"
        )
        ...
        map_data_path = os.path.join(PWD, "Data/BAF/attribute_pool/baf_data_df.csv")
        map_data = pd.read_csv(map_data_path)
        args.map_data = map_data

        whole_raw_path = os.path.join(PWD, "Data/BAF/material_data/")
        label = torch.load(os.path.join(whole_raw_path, "baf_label.pt"))
        x = torch.cat(
            [
                torch.load(
                    os.path.join(whole_raw_path, "baf_num_properties_tensor.pt")
                ),
                torch.load(
                    os.path.join(whole_raw_path, "baf_cat_properties_tensor.pt")
                ),
            ],
            dim=1,
        )

        edge_index = torch.load(os.path.join(whole_raw_path, "baf_edge_index.pt"))
        edge_type = torch.load(os.path.join(whole_raw_path, "baf_edge_type.pt"))
    ...
    def organize_val_log(self, val_loss, val_acc, val_auc, epoch):
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            print(
                "epoch: %d, val_acc: %.4f, val_loss: %.4f, val_auc: %.4f, test_acc: %.4f, test_precision: %.4f, test_recall: %.4f, test_f1: %.4f, auc_roc: %.4f%s"
                % (
                    epoch,
                    val_acc,
                    val_loss,
            ...
            self.organize_val_log(val_loss, val_acc, val_auc, epoch)
        ...
        val_aucs_np = np.array(val_aucs)
        best_idx = int(np.argmax(val_aucs_np))
        test_results = self.model.get_test_results()
        best_result = test_results[best_idx]
        print("=== Best epoch by val_auc ===")
        print(
            "best_epoch: %d, best_val_auc: %.4f, test_acc_at_best: %.4f, test_f1_at_best: %.4f, test_auc_at_best: %.4f"
            % (
                best_idx,
                val_aucs_np[best_idx],
                best_result[0],
                best_result[1],
                best_result[4],
            )
        ...
        top_k = min(self.save_top_k, len(val_aucs_np))
        top_auc_index = val_aucs_np.argsort()[::-1][:top_k]
        print("=== top-%d results by val_auc ===" % top_k)
        for idx in top_auc_index:
            result = test_results[idx]
            print(
                "epoch: %d, val_auc: %.4f, test_acc: %.4f, test_f1: %.4f, test_recall: %.4f, test_precision: %.4f, auc_roc: %.4f"
                % (
                    idx,
                    val_aucs_np[idx],
                    result[0],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="baf")
    parser.add_argument("--node_num", type=int, default=10000)
    parser.add_argument("--attribute_num", type=int, default=31)
    parser.add_argument("--num_classes", type=int, default=2)

    parser.add_argument("--feature_num", type=int, default=768)
    parser.add_argument("--prop_num", type=int, default=20)
    parser.add_argument("--cat_num", type=int, default=32)
