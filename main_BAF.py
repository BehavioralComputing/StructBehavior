...
class StructBehavior(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            return val_acc, val_loss.item(), test_acc, test_precision, test_recall , test_f1, auc_roc
...
class Trainer(object):
    ...
    def load_data(self):
        ...
        tree_path = os.path.join(
            PWD, 'Data/BAF/attribute_tree/trees',
        ...
        # Layer2 Data
        attribute_pool_graph_path = os.path.join(PWD, 'Data/BAF/attribute_pool/baf_weighted_graph.dgl')
        ...
        x = torch.cat([
            torch.load(os.path.join(whole_raw_path, 'baf_num_properties_tensor.pt')),
            torch.load(os.path.join(whole_raw_path, 'baf_cat_properties_tensor.pt')),
        ], dim = 1)
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            # Validation
            val_acc, val_loss, test_acc, test_precision, test_recall, test_f1, auc_roc = self.eval(batch)
            print(
                'epoch: %d, val_acc: %.4f, val_loss: %.4f, test_acc: %.4f, test_precision: %.4f, test_recall: %.4f, test_f1: %.4f, auc_roc: %.4f'
                % (epoch, val_acc, val_loss, test_acc, test_precision, test_recall, test_f1, auc_roc)
            )

            if auc_roc > best_test_auc:
                best_test_auc = auc_roc
