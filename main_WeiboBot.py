...
class BotRGCN(nn.Module):
    def __init__(self, args):
        ...
        self.post_prop_size = args.post_num
        ...
        self.linear_relu_num_prop = nn.Sequential(
            nn.Linear(self.num_prop_size, int(input_dimension / 3)), nn.LeakyReLU()
        )
        self.linear_relu_cat_prop = nn.Sequential(
            nn.Linear(self.cat_prop_size, int(input_dimension / 3)), nn.LeakyReLU()
        )
        self.linear_relu_post_prop = nn.Sequential(
            nn.Linear(self.post_prop_size, int(input_dimension / 3 + 2)), nn.LeakyReLU()
        )
    ...
    def forward(self, data, return_attention=False):
        ...
        post_prop = x[:, self.num_prop_size + self.cat_prop_size :]
        n = self.linear_relu_num_prop(num_prop)
        c = self.linear_relu_cat_prop(cat_prop)
        p = self.linear_relu_post_prop(post_prop)
        x = torch.cat((n, c, p), dim=1)
...
class Trainer(object):
    ...
    def load_data(self):
        ...
        attri_raw_path = os.path.join(PWD, "Data/WeiboBot/attribute_tree/mid")
        ...
        attribute_pool_graph_path = os.path.join(
            PWD, "Data/WeiboBot/attribute_pool/weibo_weighted_graph.dgl"
        )
        ...
        map_data_path = os.path.join(
            PWD, "Data/WeiboBot/attribute_pool/weibo_data_df.csv"
        )
        map_data = pd.read_csv(map_data_path)
        args.map_data = map_data

        whole_raw_path = os.path.join(PWD, "Data/WeiboBot/material_data")
        label = torch.load(os.path.join(whole_raw_path, "weibo_label.pt"))
        x = torch.cat(
            [
                torch.load(
                    os.path.join(whole_raw_path, "weibo_num_properties_tensor.pt")
                ),
                torch.load(
                    os.path.join(whole_raw_path, "weibo_cat_properties_tensor.pt")
                ),
                torch.load(os.path.join(whole_raw_path, "weibo_posts_tensor.pt")),
            ],
            dim=1,
        )

        edge_index = torch.load(os.path.join(whole_raw_path, "weibo_edge_index.pt"))
        edge_type = torch.load(os.path.join(whole_raw_path, "weibo_edge_type.pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--node_num", type=int, default=985)
    parser.add_argument("--attribute_num", type=int, default=13)
    parser.add_argument("--num_classes", type=int, default=2)

    parser.add_argument("--feature_num", type=int, default=768)
    parser.add_argument("--prop_num", type=int, default=7)
    parser.add_argument("--cat_num", type=int, default=2)
    parser.add_argument("--post_num", type=int, default=768)
