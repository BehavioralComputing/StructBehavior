...
class SEP_G(torch.nn.Module):
    ...
    def get_convs(self):
        ...
        for _ in range(self.args.tree_depth - 1):
            if self.args.conv == "GCN":
                ...
                conv = GATConv(
                    _input_dim, _output_dim, self.args.num_head, concat=False
                )
                ...
                conv = TransformerConv(_input_dim, _output_dim, 2, concat=False)
    ...
    def forward(self, batch_data):
        ...
        for _, x in enumerate(xs):
            batch = self.__process_batch(
                batch_data, min(_ + 1, self.args.tree_depth - 1)
            )
