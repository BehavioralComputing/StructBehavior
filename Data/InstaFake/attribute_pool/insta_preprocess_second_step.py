...
class PreprocessSecondStep:
    def __init__(self, root="./", device="cuda:0"):
        ...
        print(f"Loading data from {self.root}/insta_preprocessed_data_first_step.csv")
        self.df_data = pd.read_csv(
            os.path.join(self.root, "insta_preprocessed_data_first_step.csv")
        )
    ...
    def preprocess(self):
        ...
        print(f"Saving data to {self.root}/insta_preprocessed_data_second_step.csv")
        self.df_data.to_csv(
