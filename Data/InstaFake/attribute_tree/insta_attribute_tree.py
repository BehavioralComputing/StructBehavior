...
class Insta_attribute_tree(Dataset):
    def __init__(self, device="cuda:0", process=True, save=True):
        ...
        if process:
            ...
            print("Loading insta preprocessed.json")
            self.df_data = pd.read_json(
                self.raw_data_path + "/instafake_preprocessed.json"
            )

            self.df_data = self.df_data.drop(columns=["automatedBehaviour"])
            print(f"Shape of df_data: {self.df_data.shape}")
            print("---Loading Raw Json Files Finished!---")

            self.df_label = torch.load(pt_path + "/insta_label.pt")
    ...
    def train_val_test_mask(self):
        raise NotImplementedError(
            "Wrong node split, should re-implement train_val_test_mask before use."
        )
...
if __name__ == "__main__":
    dataset = Insta_attribute_tree(device=device, process=True, save=True)
