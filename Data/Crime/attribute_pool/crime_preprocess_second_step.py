...
class PreprocessSecondStep:
    def __init__(self, root="./", device="cuda:0"):
        ...
        print(f"Loading data from {self.root}crime_preprocessed_data_first_step.csv")
        self.df_data = pd.read_csv(
            os.path.join(self.root, "crime_preprocessed_data_first_step.csv")
        )
    ...
    def __all_columns_to_str(self):
        ...
        self.df_data["Vict Age"] = self.df_data["Vict Age"].apply(
            lambda x: "Unknown" if x == 0 else x
        )

        print(f"Has nan? {self.df_data.isna().any().any()}")
    ...
    def preprocess(self):
        ...
        print(f"Saving data to {self.root}crime_preprocessed_data_second_step.csv")
        self.df_data.to_csv(
