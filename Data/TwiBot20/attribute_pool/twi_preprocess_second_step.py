import pandas as pd
from misc.ToolFunc import two_significant_digits, analyse_df_feature
import os


class PreprocessSecondStep:
    def __init__(self, root="./", device="cuda:0"):
        self.root = root
        self.device = device
        self.debug = {
            "color": False,
            "bool": False,
        }

        print(f"Loading data from {self.root}/twi_preprocessed_data_first_step.csv")
        self.df_data = pd.read_csv(
            os.path.join(self.root, "twi_preprocessed_data_first_step.csv")
        )
        print("Finished loading data")

    def __approx_color(self):
        for column in self.df_data.columns:
            if "color" in column:
                print(f"\nNow processing {column}\n")
                self.df_data[column] = self.df_data[column].apply(
                    lambda x: x // 2 * 2 if pd.notna(x) and x != "None " else x
                )

    def __extract_bg_url_theme(self):
        url_column_pairs = [
            ("profile_background_image_url", "profile_background_image_url_https")
        ]

        for column, column_https in url_column_pairs:
            print(f"\nNow processing {column} and {column_https}\n")
            self.df_data[column] = self.df_data[column].apply(
                lambda x: x.split("/")[-2] if x != "Unknown" and x != "False" else x
            )
            self.df_data[column_https] = self.df_data[column_https].apply(
                lambda x: x.split("/")[-2] if x != "Unknown" and x != "False" else x
            )
            self.df_data[column] = self.df_data[column].astype("string")
            self.df_data[column_https] = self.df_data[column_https].astype("string")

    def __reduce_too_many_unique_values(self):
        threshold = 1000

        for column in self.df_data.columns:
            print(f"\nNow processing {column}\n")
            unique_size = self.df_data[column].nunique()
            if unique_size > threshold:
                print(
                    f"Column {column} with type {self.df_data[column].dtype} has {unique_size} unique values, converting to bool"
                )
                self.df_data[column] = self.df_data[column].apply(
                    lambda x: "True" if x != "Unknown" and x != "False" else x
                )
                self.df_data[column] = self.df_data[column].astype("string")

    def __all_columns_to_str(self):
        self.df_data = self.df_data.fillna("Unknown")

        self.df_data = self.df_data.replace("None ", "False")

        self.df_data = self.df_data.replace("{'description': {'urls': []}} ", "False")

        for column in self.df_data.columns:
            self.df_data[column] = self.df_data[column].apply(lambda x: str(x).strip())
            self.df_data[column] = self.df_data[column].astype("string")

    def preprocess(self):
        self.__approx_color()
        self.__all_columns_to_str()
        self.__extract_bg_url_theme()
        self.__reduce_too_many_unique_values()

        self.df_data = self.df_data.drop(
            columns=["profile_background_image_url", "profile_image_url"]
        )

        analyse_df_feature(self.df_data, threshold=1000)
        print(f"Saving data to {self.root}/twi_preprocessed_data_second_step.csv")
        self.df_data.to_csv(
            os.path.join(self.root, "twi_preprocessed_data_second_step.csv"),
            index=False,
        )


if __name__ == "__main__":
    preprocessor = PreprocessSecondStep()
    preprocessor.preprocess()
